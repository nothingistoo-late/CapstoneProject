...
public static class NotificationTemplateHelper
{
    ...
    public static NotificationRequest BuildWelcomeNotification(string contact, string fullName, NotificationChannelEnum channel, string? supportEmail = null, string? appName = null)
    {
        var templateData = new Dictionary<string, object>
        {
            ["fullName"] = fullName,
            ["contact"] = contact,
            ["appName"] = appName ?? "ChemistrySubject",
            ["supportEmail"] = supportEmail ?? "support@ChemistrySubject.com"
        };
    ...
    public static Dictionary<string, object> GetOtpTemplateData(string contact, string otpCode, OtpTypeEnum otpType, object? userData = null, int expirationMinutes = 5, string? supportEmail = null, string? appName = null)
    {
        ...
        switch (otpType)
        {
            case OtpTypeEnum.Registration:
                ...
                {
                    templateData["firstName"] = registerData.FirstName ?? string.Empty;
                    templateData["lastName"] = registerData.LastName ?? string.Empty;
                    
                    // Build fullName with fallback logic
                    var firstName = registerData.FirstName?.Trim() ?? string.Empty;
                    var lastName = registerData.LastName?.Trim() ?? string.Empty;
                    
                    if (!string.IsNullOrEmpty(firstName) || !string.IsNullOrEmpty(lastName))
                    {
                        templateData["fullName"] = $"{firstName} {lastName}".Trim();
                    }
                    // If both names are empty, keep the default name from contact
                }
                break;
    ...
    {
        var appTitle = appName ?? "ChemistrySubject";
