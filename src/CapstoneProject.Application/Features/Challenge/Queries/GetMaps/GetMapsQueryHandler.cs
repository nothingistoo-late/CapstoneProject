...
public class GetMapsQueryHandler : IRequestHandler<GetMapsQuery, Result<PaginationResult<MapListItemDto>>>
{
    ...
    public GetMapsQueryHandler(IUnitOfWork unitOfWork)
    {
    ...
    public async Task<Result<PaginationResult<MapListItemDto>>> Handle(GetMapsQuery request, CancellationToken cancellationToken)
    {
        ...
        if (request.ConceptId.HasValue)
            query = query.Where(m => m.MapConcepts.Any(c => c.ConceptId == request.ConceptId.Value));
        if (request.TagId.HasValue)
            query = query.Where(m => m.MapTags.Any(t => t.TagId == request.TagId.Value));
        if (request.CreatedByUserId.HasValue)
            query = query.Where(m => m.CreatedByUserId == request.CreatedByUserId.Value);
        ...
        var list = await query
            ...
            .Select(m => new MapListItemDto
            {
                ...
                ConceptNames = m.MapConcepts.Select(c => c.Concept.Name).ToList()
            })
