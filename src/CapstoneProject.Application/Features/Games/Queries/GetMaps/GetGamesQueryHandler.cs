...
public class GetMapsQueryHandler : IRequestHandler<GetMapsQuery, Result<PaginationResult<MapListItemDto>>>
{
    ...
    public async Task<Result<PaginationResult<MapListItemDto>>> Handle(GetMapsQuery request, CancellationToken cancellationToken)
    {
        ...
        // Status filter: if GameStatus provided use it; else if publishedOnly use Published only
        if (request.GameStatus.HasValue)
            ...
            query = query.Where(m => m.IsPublished && m.IsActiveVersion && m.GameStatus == GameStatusEnum.Published);
        ...
        if (request.TagId.HasValue) query = query.Where(m => m.GameTags.Any(t => t.TagId == request.TagId.Value));
        ...
        return Result<PaginationResult<MapListItemDto>>.Success(result, "Đã lấy danh sách trò chơi.");
    }
}
