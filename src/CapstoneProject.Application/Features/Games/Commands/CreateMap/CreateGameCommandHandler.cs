...
public class CreateMapCommandHandler : IRequestHandler<CreateMapCommand, Result<Guid>>
{
    ...
    public async Task<Result<Guid>> Handle(CreateMapCommand command, CancellationToken cancellationToken)
    {
        ...
        var game = new Game
        {
            ...
            GameStatus = command.AutoPublish ? GameStatusEnum.Published : GameStatusEnum.Draft,
            LearnedTags = req.LearnedTags,
            AvatarUrl = req.AvatarUrl,
            ContentVersion = 1
        };
        game.InitializeEntity(userId);
        game.RootGameId = game.Id;
        game.IsActiveVersion = true;

        var mapRepo = _unitOfWork.Repository<Game>();
        await mapRepo.AddAsync(game);

        var mapTagRepo = _unitOfWork.Repository<GameTag>();
        ...
        var myMap = new MyGame { GameId = game.Id, UserId = userId, IsAuthor = true };
        ...
        return Result<Guid>.Success(game.Id, message);
    }
    ...
    static List<MapLevelInputDto> ResolveLevelInputs(CreateMapRequest req)
    {
        ...
        if (req.GameDetailJson.HasValue)
