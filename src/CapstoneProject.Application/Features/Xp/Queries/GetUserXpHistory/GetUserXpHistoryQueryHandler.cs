...
public class GetUserXpHistoryQueryHandler : IRequestHandler<GetUserXpHistoryQuery, Result<PaginationResult<XpHistoryItemDto>>>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly ICurrentUserService _currentUserService;

    public GetUserXpHistoryQueryHandler(IUnitOfWork unitOfWork, ICurrentUserService currentUserService)
    {
        _unitOfWork = unitOfWork;
        _currentUserService = currentUserService;
    }

    public async Task<Result<PaginationResult<XpHistoryItemDto>>> Handle(GetUserXpHistoryQuery request, CancellationToken cancellationToken)
    {
        ...
        if (!roles.Contains(RoleEnum.Admin) && !roles.Contains(RoleEnum.Moderator))
            return Result<PaginationResult<XpHistoryItemDto>>.Failure("Only Admin/Moderator can view user XP history.", ErrorCodeEnum.Forbidden);
